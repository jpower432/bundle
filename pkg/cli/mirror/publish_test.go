...
func TestFindBlobRepo(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			meta := v1alpha2.Metadata{
				MetadataSpec: v1alpha2.MetadataSpec{
					PastMirror: v1alpha2.PastMirror{
						Association: image.AssociationSet{"registry.com/test3/baz": map[string]image.Association{
			...
			ref, err := test.options.findBlobRepo(meta.PastMirror.Association, test.digest)
			if len(test.err) != 0 {
